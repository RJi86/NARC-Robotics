...
void setup() {
  Serial.begin(115200);
  //pins
  multiplexer4.A0Pin = 38;
  multiplexer4.A1Pin = 39;
  multiplexer4.A2Pin = 40;
  multiplexer4.outputPin = 5;
...
// Function to read all light sensors and print their values
void readAllLightSensors() {
  Serial.println("Light Sensor Readings:");
  
  // Read all 8 sensors from multiplexer4
  for (int i = 0; i < 8; i++) {
    int lightValue = readLightSensor(multiplexer4, i);
    Serial.print("Sensor ");
    Serial.print(i);
    Serial.print(": ");
...
void loop() {
  ...
  delay(500);  // Delay half a second between readings
}